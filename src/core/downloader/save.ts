...
async function getURL(
    ...
    const response: { url: { url: string; filename: string } } =
        await openSubs.search({
 ...
 **/
const getFile = async(filename:string, url:string) => {

	const cwd = process.cwd()
	const path = cwd.split('/').slice(0,-2).join('/').concat('/assets/subtitles');
